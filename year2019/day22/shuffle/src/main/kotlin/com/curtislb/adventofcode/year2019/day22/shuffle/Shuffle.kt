...
interface Shuffle {
    /**
     * Returns the new position of the card at [position] after shuffling with the given [deckSize].
     */
    fun apply(position: BigInteger, deckSize: BigInteger): BigInteger

    /**
     * Returns the original position of the card at [position], before shuffling with the given
     * [deckSize].
     *
     * This method must be defined such that:
     *
     * ```
     * applyReverse(apply(position, deckSize), deckSize) == position
     * ```
     */
    ...
    companion object {
        ...
        fun from(shuffleString: String): Shuffle {
            ...
            return when {
                ...
                trimmedString.startsWith("deal with increment") -> DealWithIncrement(
                    extractParameter(trimmedString)
                )
         ...
         */
        private fun extractParameter(shuffleString: String): BigInteger =
            shuffleString.split(' ').last().toBigInteger()

        /**
         * Reverses a series of [shuffleSteps] and returns the original position of the card
         * currently at [position], given a [deckSize].
         */
        fun reverse(
            shuffleSteps: List<Shuffle>,
            position: BigInteger,
            deckSize: BigInteger
        ): BigInteger {
