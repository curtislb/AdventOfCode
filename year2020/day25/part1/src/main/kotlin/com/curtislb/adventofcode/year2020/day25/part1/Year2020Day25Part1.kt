...
- The card and door use the wireless RFID signal to transmit the two public keys (your puzzle input)
  to the other device. Now, the card has the door's public key, and the door has the card's public
  key. Because you can eavesdrop on the signal, you have both public keys, but neither device's loop
  size.
...
/**
 * Returns the solution to the puzzle for 2020, day 25, part 1.
 *
 * @param inputPath The path to the input file for this puzzle.
 */
fun solve(inputPath: Path = Paths.get("..", "input", "input.txt")): Long {
    val file = inputPath.toFile()
    val publicKeys = mutableListOf<Long>().apply { file.forEachLine { add(it.toLong()) } }
    val loopSizes = publicKeys.map { findLoopSize(it) }
    val minLoopSizeWithIndex = loopSizes.withIndex().minByOrNull { it.value }!!
    val subject = publicKeys[(minLoopSizeWithIndex.index + 1) % publicKeys.size]
    return transform(subject, minLoopSizeWithIndex.value)
}

private fun findLoopSize(result: Long, subject: Long = 7L): Int {
    var value = 1L
    var loopSize = 0
    while (value != result) {
        value = value.modMultiply(subject, 20201227L)
        loopSize++
    }
    return loopSize
}
...
private fun transform(subject: Long, loopSize: Int): Long {
    var value = 1L
    repeat(loopSize) { value = value.modMultiply(subject, 20201227L) }
    return value
}
