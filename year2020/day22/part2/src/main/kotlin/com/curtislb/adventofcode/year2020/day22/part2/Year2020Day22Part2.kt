...
- As in regular Combat, the winner of the round (even if they won the round by winning a sub-game) takes the two cards
  ...
  due to winning a sub-game. If collecting cards by winning the round causes a player to have all of the cards, they
...
private fun playGame(deck1: ArrayDeque<Int>, deck2: ArrayDeque<Int>): Pair<Boolean, Int> {
    ...
    while (deck1.isNotEmpty() && deck2.isNotEmpty()) {
        ...
        val isPlayer1RoundWinner = if (deck1.size >= card1 && deck2.size >= card2) {
            ...
            val (isPlayer1RecursiveWinner, _) = playGame(ArrayDeque(recursiveDeck1), ArrayDeque(recursiveDeck2))
...
private fun calculateScore(deck: List<Int>): Int = deck.mapIndexed { index, card -> (deck.size - index) * card }.sum()
