...
brought a small deck of space cards! You'd like to play a game of Combat, and there's even an
...
own deck so that the winner's card is above the other card. If this causes a player to have all of
...
worth the value of the card multiplied by 1, the second-from-the-bottom card is worth the value of
the card multiplied by 2, and so on. With 10 cards, the top card is worth the value on the card
...
fun solve(inputPath: Path = Paths.get("..", "input", "input.txt")): Int {
    ...
    while (decks.count { it.isNotEmpty() } > 1) {
        ...
        for (i in decks.indices) {
            ...
            index = (index + 1) % decks.size
        }
    }

    val winningDeck = decks.find { it.isNotEmpty() }
    return winningDeck?.mapIndexed { index, card -> (winningDeck.size - index) * card }?.sum() ?: 0
}
