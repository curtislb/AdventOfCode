...
not valid for any field. Adding together all of the invalid values produces your ticket scanning error rate: 4 + 55 + 12
= 71.
 ...
 */
fun solve(inputPath: Path = pathToInput(year = 2020, day = 16)): Int {
...
private fun parseFieldsSection(lines: List<String>): List<IntRange> {
    return lines.flatMap { line ->
        ...
        matchResults.map { result -> result.value.toIntRange(separator = "-") }
    }
}

private fun parseNearbyTicketsSection(lines: List<String>): List<Ticket> {
    return lines.subList(1, lines.size).map { line -> Ticket(line.toInts(separator = ",")) }
}
