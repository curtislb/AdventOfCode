...
/**
 * A collection of distinct joltage adapters that may be chained to connect a compatible device to an outlet.
 *
...
class JoltageAdapters(ratings: Set<Int>) {
    ...
    /**
     * Returns the count of each joltage rating difference between subsequent adapters in a sorted adapter chain.
     *
     * Adapters are sorted in increasing order by joltage rating and connected to an outlet with an effective rating of
     * 0 jolts. The difference between the minimum adapter rating and the outlet's rating is included in the result.
     */
    ...
    /**
     * Returns the number of distinct ways that these adapters can be arranged to connect a device to an outlet.
     *
     * A valid arrangement consists of a subset of adapters connected to an outlet in increasing order by joltage
     * rating, such that the difference in ratings between any two connected adapters is at most 3 jolts. For this
     * purpose, the outlet is treated as an adapter with an effective rating of 0 jolts.
     */
