...
- `[[[[[9,8],1],2],3],4]` becomes `[[[[0,9],2],3],4]` (the 9 has no regular number to its left, so
...
- `[[3,[2,[1,[7,3]]]],[6,[5,[4,[3,2]]]]]` becomes `[[3,[2,[8,0]]],[9,[5,[4,[3,2]]]]]` (the pair
  `[3,2]` is unaffected because the pair `[7,3]` is further to the left; [3,2] would explode on the
  next action).
...
package com.curtislb.adventofcode.year2021.day18.part1
...
/**
 * Returns the solution to the puzzle for 2021, day 18, part 1.
 *
 * @param inputPath The path to the input file for this puzzle.
 */
fun solve(inputPath: Path = Paths.get("..", "input", "input.txt")): Int? {
    var snailfishSum: SnailfishNumber? = null
    inputPath.toFile().forEachLine { line ->
        val number = SnailfishNumber.fromString(line)
        snailfishSum = snailfishSum?.let { it + number } ?: number
    }
