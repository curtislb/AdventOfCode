...
So, the coordinate `3,0` is to the right of `0,0`, and the coordinate `0,7` is below `0,0`. The
coordinates in this example form the following pattern, where `#` is a dot on the paper and `.` is
...
`x=...` lines). In this example, the first fold instruction is fold along `y=7`, which designates
...
after the fold is complete, those dots appear in the top left corner (at `0,0` and `0,1`). Because
the paper is transparent, the dot just below them in the result (at `0,3`) remains visible, as it
can be seen through the transparent paper.
...
/**
 * Returns the solution to the puzzle for 2021, day 13, part 1.
 *
 * @param inputPath The path to the input file for this puzzle.
 */
fun solve(inputPath: Path = Paths.get("..", "input", "input.txt")): Int {
    val (pointStrings, instructionStrings) = inputPath.toFile().readSections()
    val sheet = OrigamiSheet(pointStrings, instructionStrings).apply { fold(instructionCount = 1) }
    return sheet.points.size
}
