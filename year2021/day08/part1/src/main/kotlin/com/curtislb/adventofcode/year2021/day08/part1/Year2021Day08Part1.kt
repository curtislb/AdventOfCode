...
So, to render a 1, only segments `c` and `f` would be turned on; the rest would be off. To render a
7, only segments `a`, `c`, and `f` would be turned on.

The problem is that the signals which control the segments have been mixed up on each display. The
submarine is still trying to display numbers by producing output on signal wires `a` through `g`,
...
`b` and `g` are turned on: the only digit that uses two segments is 1, so it must mean segments `c`
and `f` are meant to be on. With just that information, you still can't tell which wire (`b`/`g`)
...
output value. Within an entry, the same wire/segment connections are used (but you don't know what
the connections actually are). The unique signal patterns correspond to the ten different ways the
submarine tries to render a digit using the current wire/segment connections. Because 7 is the only
digit that uses three segments, `dab` in the above example means that to render a 7, signal lines
`d`, `a`, and `b` are on. Because 4 is the only digit that uses four segments, `eafb` means that to
...
above example, all of the digits in the output value (`cdfeb` `fcadb` `cdfeb` `cdbaf`) use five
...
package com.curtislb.adventofcode.year2021.day08.part1
...
/**
 * Returns the solution to the puzzle for 2021, day 8, part 1.
 *
...
fun solve(inputPath: Path = Paths.get("..", "input", "input.txt")): Int {
    ...
    // Count all output patterns with unique segment counts
    var count = 0
    inputPath.toFile().forEachLine { line ->
        count += SevenSegmentDisplay(line).outputPatterns.count { it.countWires() in uniqueCounts }
    }
    return count
}
