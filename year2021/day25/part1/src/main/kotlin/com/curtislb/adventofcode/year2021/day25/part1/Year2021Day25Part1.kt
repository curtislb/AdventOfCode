...
There are two herds of sea cucumbers sharing the same region; one always moves east (`>`), while the
other always moves south (`v`). Each location can contain at most one sea cucumber; the remaining
locations are empty (`.`). The submarine helpfully generates a map of the situation (your puzzle
input). For example:
...
Every step, the sea cucumbers in the east-facing herd attempt to move forward one location, then the
...
import com.curtislb.adventofcode.year2021.day25.cucumber.SeaCucumberField
import java.nio.file.Path
import java.nio.file.Paths

/**
 * Returns the solution to the puzzle for 2021, day 25, part 1.
 *
 * @param inputPath The path to the input file for this puzzle.
 */
fun solve(inputPath: Path = Paths.get("..", "input", "input.txt")): Int {
    val seaCucumberField = SeaCucumberField.fromFile(inputPath.toFile())
    return seaCucumberField.countStepsUntilNoMovement()
}
