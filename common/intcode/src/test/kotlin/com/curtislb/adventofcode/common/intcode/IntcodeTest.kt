...
class IntcodeTest {
    ...
    fun testConstructFromFile(@TempDir tempDir: Path) {
        val file = tempDir.resolve("tmp").toFile().apply { writeText("4,5,4,6,99,-2,11") }
    ...
    @Test
    fun testConstructFromFileWithCustomOutputFunction(@TempDir tempDir: Path) {
        val file = tempDir.createTempFile(text = "4,5,4,6,99,3,13")
