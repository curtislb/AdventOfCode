...
function (as the start and end, respectively); all other outer labeled tiles are effectively walls. At any other level,
AA and ZZ count as walls, but the other outer labeled tiles bring you one level outward.
...
Because this is not the outermost level, ZZ is a wall, and the only option is to go back around to BC, which would only
send you even deeper into the recursive maze.
...
fun main() {
    ...
    val distance = maze.findShortestDistance(ENTRANCE_LABEL, EXIT_LABEL, isRecursive = true)
    if (distance != null) {
        println(distance)
    } else {
