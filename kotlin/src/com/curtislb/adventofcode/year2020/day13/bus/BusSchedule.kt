 ...
 *
 * All in-service buses initially depart at timestamp 0, and the ID of each bus indicates the time between this and each
 * subsequent departure.
 *
 * @param schedule A string representation of the schedule, with integer bus IDs (or out-of-service buses, represented
 *  by `x`) separated by commas.
 */
class BusSchedule(schedule: String) {
    ...
    /**
     * Finds the first bus to depart after (or at) [arrivalTime] and returns a pair containing its ID and the number of
     * minutes after [arrivalTime] that it departs.
     */
    ...
    /**
     * Returns the earliest time `t` (in minutes) such that each in-service bus with schedule index `i` departs at time
     * `t + i`.
     */
    fun findEarliestAlignedDepartureTime(): Long {
        val (time, _) = busIDs.entries.sortedBy { it.value }.foldRight(Pair(0L, 1L)) { (offset, busID), (time, step) ->
            var newTime = time
