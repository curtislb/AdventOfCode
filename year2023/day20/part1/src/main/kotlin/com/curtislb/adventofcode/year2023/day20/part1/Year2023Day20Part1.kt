...
machines directly, but rather to communication modules attached to the machines that perform various
...
Each of these modules is a flip-flop module (as indicated by the `%` prefix). `a` outputs to `b`
...
the pulse sent to `con` is handled first, so it briefly remembers all high pulses for its inputs and
...
high pulses are sent. So, after pushing the button 1000 times, 8000 low pulses and 4000 high pulses
...
/**
 * Returns the solution to the puzzle for 2023, day 20, part 1.
 *
 ...
 * @param pressCount The total number of times the button should be pressed.
 */
fun solve(
    ...
    // Build the processor, and prepare to count the numbers of high and low pulses
    var highPulseCount = 0L
    var lowPulseCount = 0L
    val processor = PulseProcessor.fromFile(inputPath.toFile()).apply {
    ...
    // Press the button a fixed number of times, then multiply the total pulse counts
