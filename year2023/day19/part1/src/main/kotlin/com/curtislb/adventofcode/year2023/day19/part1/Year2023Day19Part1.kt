...
and never returns. If a part is accepted (sent to `A`) or rejected (sent to `R`), the part
...
The workflows are listed first, followed by a blank line, then the ratings of the parts the Elve
...
package com.curtislb.adventofcode.year2023.day19.part1

import com.curtislb.adventofcode.common.io.forEachSection
import com.curtislb.adventofcode.year2023.day19.workflow.MachinePart
...
/**
 * Returns the solution to the puzzle for 2023, day 19, part 1.
 *
...
fun solve(
    ...
    inputPath.toFile().forEachSection { lines ->
        if (workflows.isEmpty()) {
            ...
            // Run the sorting process for all machine parts
            val process = SortingProcess(workflows)
            for (line in lines) {
                val part = MachinePart.fromString(line)
                if (process.isAccepted(part, startWorkflow)) {
                    // Add the accepted part's rating sum to the total
                    totalRatings += part.sumRatings()
                }
            }
        }
    }
