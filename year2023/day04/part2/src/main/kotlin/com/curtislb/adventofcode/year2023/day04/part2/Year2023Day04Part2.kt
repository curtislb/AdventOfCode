...
import com.curtislb.adventofcode.year2023.day04.scratchcard.Scratchcard
import java.nio.file.Path
import java.nio.file.Paths

/**
 * Returns the solution to the puzzle for 2023, day 4, part 2.
 *
 * @param inputPath The path to the input file for this puzzle.
 */
fun solve(inputPath: Path = Paths.get("..", "input", "input.txt")): Int {
    val cards = inputPath.toFile().mapLines { Scratchcard.fromString(it) }

    // Set card counts to 1, then add copies based on the number of matches for each card
    val cardCounts = IntArray(cards.size) { 1 }
    for ((index, card) in cards.withIndex()) {
        val matchCount = card.countMatches()
        val copyIndexStart = index + 1
        val copyIndexEnd = (index + matchCount).coerceAtMost(cards.lastIndex)
        for (copyIndex in copyIndexStart..copyIndexEnd) {
            cardCounts[copyIndex] += cardCounts[index]
        }
    }
