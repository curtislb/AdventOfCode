...
import com.curtislb.adventofcode.year2023.day10.pipes.PipeMaze
import java.nio.file.Path
import java.nio.file.Paths

/**
 * Returns the solution to the puzzle for 2023, day 10, part 2.
 *
 * @param inputPath The path to the input file for this puzzle.
 */
fun solve(inputPath: Path = Paths.get("..", "input", "input.txt")): Long? {
    val maze = PipeMaze.fromFile(inputPath.toFile())
    val loop = maze.findLoop() ?: return null

    // Find the number of interior points using Pick's theorem
    val area = polygonArea(loop)
    return area - (loop.size / 2L) + 1L
}
