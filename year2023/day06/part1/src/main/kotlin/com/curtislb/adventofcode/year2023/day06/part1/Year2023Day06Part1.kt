...
package com.curtislb.adventofcode.year2023.day06.part1
...
/**
 * Returns the solution to the puzzle for 2023, day 6, part 1.
 *
 * @param inputPath The path to the input file for this puzzle.
 */
fun solve(inputPath: Path = Paths.get("..", "input", "input.txt")): Long {
    // Read race times and distances from file
    val (timeLine, distanceLine) = inputPath.toFile().readLines()
    val times = timeLine.parseLongs()
    val distances = distanceLine.parseLongs()
    require(times.size == distances.size) {
        """
            Times and distances must have the same size:
            times = $times
            distances = $distances
        """.trimIndent()
    }

    // Multiply the numbers of ways to win all races together
    val races = times.mapIndexed { index, time -> BoatRace(time, distances[index]) }
    return races.productOf { it.countWaysToWin() }
}
