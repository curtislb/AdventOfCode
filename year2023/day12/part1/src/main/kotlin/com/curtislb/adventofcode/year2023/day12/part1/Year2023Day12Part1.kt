...
we get more lava to heat our forges. And our springs. The springs aren't very springy unless they're
hot!"
...
groups are always separated by at least one operational spring: `####` would always be `4`, never
`2,2`).
...
be both broken springs or they would form a single contiguous group of two; if that were true, the
numbers afterward would have been `2,3` instead.) Since each `??` can either be `#.` or `.#`, there
...
the first and second `?` must both be `.` (if either were `#`, the first number would have to be 4
or higher). However, the remaining run of unknown spring conditions have many different ways they
...
package com.curtislb.adventofcode.year2023.day12.part1
...
/**
 * Returns the solution to the puzzle for 2023, day 12, part 1.
 *
...
fun solve(inputPath: Path = Paths.get("..", "input", "input.txt")): Long {
    ...
    inputPath.toFile().forEachLine { line ->
        val springRecord = SpringRecord.fromString(line)
